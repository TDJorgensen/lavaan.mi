...
lavaan.mi <- function(model, data, ...) {
  ...
  ## First check installed version to select pre/post-lavaan 0.7 argument names
  ...
  if (lav7) names(lavListCall)[names(lavListCall) == "dataList"] <- "data_list"
  lavListCall[[ifelse(lav7, "store_slots", "store.slots")]] <- union(c("partable","vcov","test","h1","baseline"),
                                                                     lavListCall$store.slots)
  funcName <- ifelse(lav7, "fun", "FUN")
  lavListCall[[funcName]] <- if (is.null(dots[[funcName]])) .getOutput. else function(obj) {
    temp1 <- .getOutput.(obj)
    temp2 <- dots[[funcName]](obj)
  ...
  ## keep any remaining funList slots (if users supply custom function)
